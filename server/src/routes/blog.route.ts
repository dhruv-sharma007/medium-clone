...
blogRouter.use('/*', async (c, next) => {
    ...
    const isVerified: any = await verify(String(token), String(c.env.JWT_SECRET), "HS256")

    if (typeof isVerified === 'object' && isVerified !== null) {
        c.set('user', { id: isVerified.id, name: isVerified.name, username: isVerified.username })
    } else {
...
blogRouter.post('/post', async (c) => {
    try {
        ...
        await prisma.blog.create({ data: { content: body.content, title: body.title, authorId: c.get('user').id } })
        c.status(201)
...
blogRouter.put('/update', async (c) => {
    ...
    prisma.blog.update({ where: { id: body.id }, data: { title: body.title, content: body.content } })
...
blogRouter.get('/bulk', async (c) => {
    try {
        ...
        c.status(200)
        return c.json({ data: blogs, message: "Blogs found successfully" })
    } catch (error: any) {
        c.json({ message: error.message })
        return c.status(500)
    }
